...
for($i=1; $i<=6; $i++) {
	...
	$tlbl='tog'.$i."_lbl";
	$table.="<tr><td>{$lbl}</td><td colspan=\"2\">{$ov}</td><td><input type=\"text\" name=\"{$tlbl}\" value=\"{$row[$tlbl]}\" size=\"{$ss}\"></td></tr>\n";
}

$table.="<tr><td colspan=\"4\"><input type=\"submit\"></td></tr>\n";
if(DEBUG) {
	$table.="<tr><td colspan=\"4\">{$debug}</td></tr>\n";
} 
?>
<!DOCTYPE html>
<html>
